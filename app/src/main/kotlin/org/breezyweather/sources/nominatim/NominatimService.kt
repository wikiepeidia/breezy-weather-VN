...
class NominatimService @Inject constructor(
    ...
    override fun requestLocationSearch(
        ...
        val configuredInstance = configuredInstanceValue
    ...
    private val configuredInstanceValue: String?
        get() = config.getString("instance", null)?.trim()?.takeIf { it.isNotEmpty() }

    private var instance: String?
        set(value) {
            val normalized = value?.trim()?.takeIf { it.isNotEmpty() }
            if (normalized.isNullOrEmpty() || normalized == NOMINATIM_BASE_URL) {
                config.edit().remove("instance").apply()
            } else {
                config.edit().putString("instance", normalized).apply()
            }
        }
        get() = configuredInstanceValue ?: NOMINATIM_BASE_URL
    ...
    companion object {
        ...
        internal fun isLocationIqKey(value: String?): Boolean {
            return value?.trim()?.startsWith("pk.") == true
        }
